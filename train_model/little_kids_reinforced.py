...
import train_model.warp_little_kids as game
...
def network():
    ...
    fci = Flatten()(conv3)
    fc1 = Dense(512, activation='relu', use_bias=True, kernel_initializer=initializer.TruncatedNormal(stddev=0.01),
                bias_initializer=initializer.Constant(value=0.01))(fci)
    fc2 = Dense(ACTIONS, activation='linear', use_bias=True,
...
def train():
    ...
    while 1:
        ...
        if B != 1:
            if t % 25000 == 0:
                model.save_weights('trained_model/' + 'dq2n' + str(t) + '.h5')
                print("Time : ", t)
